...
class TermTable {
  ...
  TermTable(final LinkedHashSet<Term> terms) {
    this.lineHeaders = new Vector<Term>(terms.size());
    this.columnHeaders = initializeColumnHeaders(terms);
    this.matrixLines = new Vector<Vector<Boolean>>(terms.size());
    this.matrixColumns = new Vector<Vector<Boolean>>(this.columnHeaders.size());
    final Vector<Boolean> matrixLineTemplate = new Vector<Boolean>(this.columnHeaders.size());
    for (int i = 0; i < this.columnHeaders.size(); i++)
      matrixLineTemplate.add(false);
    final Vector<Boolean> matrixColumnTemplate = new Vector<Boolean>(terms.size());
    for (int i = 0; i < terms.size(); i++)
      matrixColumnTemplate.add(false);
    for (int i = 0; i < this.columnHeaders.size(); i++)
      this.matrixColumns.add(new Vector<Boolean>(matrixColumnTemplate));
    int count = 0;
    for (final Term term : terms) {
      this.lineHeaders.add(term);
      final Vector<Boolean> matrixLine = new Vector<Boolean>(matrixLineTemplate);
  ...
  private Vector<Formula> initializeColumnHeaders(final LinkedHashSet<Term> terms) {
    final LinkedHashSet<Formula> header = new LinkedHashSet<Formula>();
    for (final Term term : terms)
  ...
  private boolean eliminateColumnDominance() {
    final SortedSet<Integer> toEliminate = new TreeSet<Integer>();
  ...
  public String toString() {
    final Vector<String> lineHeaderStrings = new Vector<String>();
    ...
    for (int i = 0; i < lineHeaderSize; i++)
      sb.append(" ");
    sb.append(" | ");
    final LinkedHashMap<String, String> legend = new LinkedHashMap<String, String>();
    int columnSize = 0;
    for (int i = 0; i < this.columnHeaders.size(); i++) {
